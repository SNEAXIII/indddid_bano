"""Chapitre 10 du workflow BANO : recherche tolérante aux fautes."""

from workflow_pipeline_shared import COL_SCORE, COL_SRC, COL_TOK, QUERY_TYPO, COL_INDEX
from anim_common import MONO, titled
from workflow_pipeline_ch7 import node, edge  # mêmes états/arêtes que l'arbre du ch.7
...
class Ch10Levenshtein(Scene):
    """Chapitre 10 — on cherche même avec une faute : distance de Levenshtein."""

    def construct(self):
        title, sub = titled(
            "On cherche, même avec une faute", "distance de Levenshtein", chapter=10
        )
        self.sub = sub
