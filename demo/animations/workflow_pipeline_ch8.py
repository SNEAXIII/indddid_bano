"""Chapitre 8 du workflow BANO : la requête de l'utilisateur, côté Rust.
...
class Ch8Requete(Slide):
    """Chapitre 8 — la requête de l'utilisateur : normalise, tokenise, ET."""

    def construct(self):
        self.title, self.sub = titled(
            "Quand l'utilisateur cherche", "fn search() — en trois temps", chapter=8
        )
        self.play(Write(self.title), run_time=1.0)
        self.play(FadeIn(self.sub), run_time=0.6)

        # Points d'arrêt placés DANS les scènes, juste avant chaque FadeOut.
    ...
    def scene_tokenise(self):
        ...
        toks_cap.next_to(toks, RIGHT, buff=0.45)
        ...
        self.wait(1.0)
        self.next_slide()  # pause sur la requête normalisée + découpée
    ...
    def scene_par_mot(self):
        ...
        self.wait(1.2)
        self.next_slide()  # pause sur « faute corrigée » (chaque mot trouvé)
    ...
    def scene_intersection(self):
        ...
        self.wait(1.6)
        self.next_slide()  # pause sur le résultat de l'intersection ET

        self.play(
            FadeOut(VGroup(rows, *flash, res, why)),
            run_time=0.6,
        )
