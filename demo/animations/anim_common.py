...
def titled(title: str, subtitle: str, chapter: int | None = None):
    """Titre grand public en haut + sous-titre gris qui NOMME le terme exact
    (registre « semi-technique nommé »). Si `chapter` est fourni, ajoute un
    bandeau « Chapitre N » au-dessus du titre. Renvoie deux mobjects : le bloc
    titre (VGroup si numéroté, sinon Text) et le sous-titre."""
    t = Text(title, font_size=32)
    t.to_edge(UP, buff=0.5)
    if chapter is not None:
        # L'eyebrow se loge dans la marge haute, AU-DESSUS du titre : le titre
        # (et donc tout le contenu ancré dessous) garde sa position d'origine.
        tag = Text(f"Chapitre {chapter}", font_size=18, color=GREY_B)
        tag.next_to(t, UP, buff=0.12)
        head = VGroup(tag, t)
    else:
        head = t
    s = Text(subtitle, font_size=22, color=GREY_B)
