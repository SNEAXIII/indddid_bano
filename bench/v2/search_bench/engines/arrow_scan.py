# search_bench/engines/arrow_scan.py
import os
import tempfile
import time
...
def _read_arrow(path: str) -> pa.Table:
    with pa.memory_map(path, "r") as source:
        return pa.ipc.open_file(source).read_all()


class ArrowScan(SearchEngine):
    name = "arrow_scan"
    supports_fuzzy = True
    ...
    def build(self, records: list[Record]) -> None:
        ...
        path = os.path.join(tempfile.gettempdir(), f"bano_bench_{self.name}.arrow")
        _write_arrow(_records_to_table(records), path)
        loaded = _read_arrow(path)
    ...
    def save(self, artifact_dir: str) -> None:
        ...
        _write_arrow(
            _records_to_table(self._records), os.path.join(artifact_dir, "data.arrow")
        )
        self.artifact_size_bytes = dir_size_bytes(artifact_dir)
        write_meta(artifact_dir, self.build_time_ms, self.artifact_size_bytes)

    @classmethod
    def load(cls, artifact_dir: str) -> "ArrowScan":
        engine = cls()
        table = _read_arrow(os.path.join(artifact_dir, "data.arrow"))
